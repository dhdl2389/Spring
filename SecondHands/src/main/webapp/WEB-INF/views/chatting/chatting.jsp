...
<script type="text/javascript">
	...
	function appendRecvMessage(msg, sender) {
		if (sender != selectedNickname) {
			$("#chatMessageArea").append(
					"<div class='recv'>" + "<div class='recvNickname'>"
							+ $('#targetNickname').val() + "님 " + "</div>"
							+ "</div>");
			$("#chatMessageArea").append(
					"<div class='recv'>" + "<div class='recvBox'>" + msg
							+ "</div>" + "</div>");
	...
	function appendSendMessage(sender, msg) {
		if (sender != selectedNickname) {
			$("#chatMessageArea").append(
					"<div class='send'>" + "<div class='sendBox'>" + msg
							+ "</div>" + "</div>");
...
<body>
	...
	<c:set var="TargetNickNameCheck"
		value="${user.user_nickname eq targetNickName}" />
...
-->
	...
	<input type="hidden" id="nickname"
		value="${nickNameCheck ? nickName : targetNickName}">
	<input type="hidden" id="targetNickname"
		value="${TargetNickNameCheck ? nickName : targetNickName}">

	<input type="hidden" id=my_heat
		value="${my_heatCheck ? my_heat : target_heat}">
	<input type="hidden" id="target_heat"
		value="${target_heatCheck ? my_heat : target_heat}">
	...
	<div class="chattingMainBox">
		...
		<div class="chatMessageBox">
			<input type="text" id="message"> <input
				class="chatMessageBoxButton" type="button" id="sendBtn" value="전송"
				onclick="send()"> <input class="chatMessageBoxButton"
				type="button" id="connectBtn" value="입장" onclick="connect()">
			<input class="chatMessageBoxButton" type="button" id="exitBtn"
				value="나가기" onclick="disconnect()" style="display: none">
		</div>
	</div>
